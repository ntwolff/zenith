...
class IpAddressService(BaseService):
    def upsert(self, ip_address: IpAddress):
        query = """
            MERGE (i:IpAddress {ip_address_id: $id})
            ON CREATE SET i += $properties
            ON MATCH SET i += $properties
        """
        self.db.execute_query(query, id=ip_address.ip_address_id, properties=ip_address.asdict())

    def mark_as_risky(self, ip_address_id: str, reason: str):
        query = """
            MATCH (i:IpAddress {ip_address_id: $ip_address_id})
