...
def test_process_customer_registration_event(mock_graph_database):
    processor = RegistrationEventProcessor(mock_graph_database)
    ...
    assert mock_graph_database.create_customer_event_relationship.called
    assert mock_graph_database.create_device_node.called
    assert mock_graph_database.create_ip_address_node.called
    assert mock_graph_database.create_address_node.called
    ...
    assert mock_graph_database.create_customer_address_relationship.called
